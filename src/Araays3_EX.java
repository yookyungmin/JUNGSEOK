...
//		 int[] iArr3= new int[]{100, 95, 90, 70, 60};;
		 ...
		 for(int i=0; i<iArr1.length; i++) {
			 iArr1[i] = i+1; // 1~10숫자를 순서대로 배열에 놓는다.
		 }
		 for(int i=0; i<iArr2.length; i++) {
			 iArr2[i] = (int)(Math.random()*10)+1;  //1~10 배열의 값
		 }
		 //배열에 저장된 값들을 출력
		 for(int i=0; i<iArr1.length; i++) {
           ...
           System.out.println(iArr3); // [I@41a4555e  41a4555e = 주소값  
           System.out.println(chArr); //abcd  char캐릭터 배열인경우 프린트 내용 출력
